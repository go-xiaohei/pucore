...
// Static provides static asset service with dynamic options setting as tango middleware.
type Static struct {
	Options tango.StaticOptions
}

// copy from tango static.go
func prepareStaticOptions(options ...tango.StaticOptions) tango.StaticOptions {
	var opt tango.StaticOptions
	if len(options) > 0 {
		opt = options[0]
	}
